...
public class Isep {
	...
	public static void main(String[] args) {
		...
		if (multi) {//Toute cette partie correspond au mode multi à 2 joueur
			while (true) {
				StdDraw.clear();
				...
				for (int i = 0; i != myrectangle.size(); i++) {
					...
					myrectangle.get(i).show();// methode qui permet le
												// defilement de chaque
				...
				if (mineJ1 == false) {
					if (StdDraw.isKeyPressed(517)) {// 517 correspond à la
													// touche "!"

						myMines.add(new Mines(myVaisseau.get(0).getx(),
								myVaisseau.get(0).gety(), Missile
										.getvxmissile(), 0, 0, myVaisseau
										.get(0).getMat(), Terrain.speed, 4));// Deux
																				// paramètres
																				// ont
																				// été
																				// rajoutés
				...
				for (int i = 0; i != myVaisseau.size(); i = i + 1) {
					...
					if (myVaisseau.get(i).getlife() <= 0) {// Condition de fin
															// de partie
				 ...
				 * (myMines.get(i))
				 * .demiparabole(myVaisseau.get(0).getx(),myVaisseau
				 * .get(0).gety(), -10, 10);
				 * 
			...
			//	vaisseauBleu.setMissile(false);
				...
				for (int i = 0; i != myVaisseau.size(); i = i + 1) {
					...
					if(i==1)//Attribution de la couleur bleu au vaisseau qui fait l'A.I
						(myVaisseau.get(i)).paint(3);
					else
						(myVaisseau.get(i)).paint(i+1);
					(myVaisseau.get(i)).bordure();
					// (myVaisseau.get(i)).move();
