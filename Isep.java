...
public class Isep {
	...
	public static void main(String[] args){
		...
		while (true) {
			...
			for(int i=0;i!=myrectangle.size();i++){
				...
				for(int k=0;k<=5;k++){ //boucle pour les ralentissements
					...
					if(myVaisseau.get(0).getScore()<=tab[k]+10100 && myVaisseau.get(0).getScore()>=tab[k]+9900){
						myrectangle.get(i).setSpeed(110);//on la r�augmente (10000 unit�s de score plus tard)
					}
				}
			}
			//***************************************************
			...
			if (missile == false) {
				if (StdDraw.isKeyPressed(32)) {//L'id�e est qu'en restant appuy� sur espace il y aura seulement un ajout � la liste puisque missile sera "tru
